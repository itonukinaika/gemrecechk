 ...
 *
 * ---
 * Acknowledge: This code was developed with the assistance of Gemini 3 Pro
 * (Google AI).
 */
...
cJSON *BuildResponseSchema() {
  ...
  cJSON_AddStringToObject(deficiency, "description",
                          "不備の指摘内容。不備がない場合はOKを設定。");
...
char *BuildPatientsJsonString(int *indices, int count) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    // 各レセプトの病名一覧
    cJSON_AddStringToObject(j_item, "disease_list",
                            p->Disease_List ? p->Disease_List : "");
    // 各レセプトの診療行為一覧
    cJSON_AddStringToObject(j_item, "medical_info",
                            p->Medical_Info_List ? p->Medical_Info_List : "");
...
char *BuildGeminiPayload(int *indices, int count) {
  ...
  char *combined_instr = (char *)malloc(strlen(g_settings.GEMSysPrompt) +
                                        strlen(g_settings.GEMUserPrompt) + 10);
  sprintf(combined_instr, "%s\n\n%s", g_settings.GEMSysPrompt,
          g_settings.GEMUserPrompt);
  ...
  if (g_settings.Priority) {
    cJSON_AddStringToObject(root, "service_tier", "priority");
  }
...
char *CallGeminiAPI(const char *payload) {
  ...
  for (int attempt = 1; attempt <= max_retries; attempt++) {
    ...
    HINTERNET hSession =
        WinHttpOpen(L"gemrecechk/1.0", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY,
                    WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, 0);

    // WinHttpのIPv6 Fast Fallback
    // オプションを有効化(オンライン資格確認との兼ね合いで、必須)
    DWORD dwFastFallback = 1;
    WinHttpSetOption(hSession, WINHTTP_OPTION_IPV6_FAST_FALLBACK,
                     &dwFastFallback, sizeof(dwFastFallback));
    // Geminiの応答には時間がかかるので、WinHttpのタイムアウトを長めに設定
    // (受信は120秒)
    WinHttpSetTimeouts(hSession, 60000, 60000, 60000, 120000);

    HINTERNET hConnect =
        WinHttpConnect(hSession, GEMINI_HOST, INTERNET_DEFAULT_HTTPS_PORT, 0);

    WCHAR urlPath[1024];
    LPWSTR wModel = Utf8ToUtf16(g_settings.GeminiModel);
    LPWSTR wKey = Utf8ToUtf16(g_settings.GeminiKey);
    swprintf(urlPath, 1024, L"/v1beta/models/%s:generateContent?key=%s", wModel,
             wKey);
    free(wModel);
    free(wKey);

    HINTERNET hRequest =
        WinHttpOpenRequest(hConnect, L"POST", urlPath, NULL, WINHTTP_NO_REFERER,
                           WINHTTP_DEFAULT_ACCEPT_TYPES, WINHTTP_FLAG_SECURE);

    LPCWSTR header = L"Content-Type: application/json\r\n";
    BOOL bResults = WinHttpSendRequest(hRequest, header, -1, (LPVOID)payload,
                                       strlen(payload), strlen(payload), 0);

    if (bResults) {
      bResults = WinHttpReceiveResponse(hRequest, NULL);
      if (!bResults && g_settings.Debug) {
        DebugPrintf(
            "[Attempt %d/%d] WinHttpReceiveResponse Failed. Error: %lu\n",
    ...
    if (bResults) {
      ...
      do {
        ...
        if (WinHttpReadData(hRequest, (LPVOID)temp_buf, dwSize,
                            &dwDownloaded)) {
          temp_buf[dwDownloaded] = '\0';
          response_buffer =
              (char *)realloc(response_buffer, totalSize + dwDownloaded + 1);
      ...
      if (!bReadSuccess) {
        ...
        if (g_settings.Debug)
          DebugPrintf("[Attempt %d/%d] Failed while reading data.\n", attempt,
                      max_retries);
      }
    }
...
BOOL CheckGeminiConnection() {
  ...
  // 短めのタイムアウトで疎通性を確認する
  DWORD dwFastFallback = 1;
  WinHttpSetOption(hSession, WINHTTP_OPTION_IPV6_FAST_FALLBACK, &dwFastFallback,
                   sizeof(dwFastFallback));
...
unsigned __stdcall RunGeminiCheckThread(void *param) {
  ...
  if (!CheckGeminiConnection()) {
    MessageBoxW(args->hProgressDlg,
                L"Gemini "
                L"APIサーバーに接続できません。\nインターネット接続を確認してく"
                L"ださい。",
                L"接続エラー", MB_OK | MB_ICONERROR);
    goto THREAD_ABORT;
  }
  ...
  for (int start_idx = 0; start_idx < args->target_count;
       start_idx += split_case) {
    if (WaitForSingleObject(g_hStopEvent, 0) == WAIT_OBJECT_0)
      goto THREAD_ABORT;

    int current_chunk_size = args->target_count - start_idx;
    if (current_chunk_size > split_case)
      current_chunk_size = split_case;
    ...
    if (start_idx > 0) {
      ...
      remain_sec =
          (int)((avg_time * (args->target_count - start_idx)) / 1000.0f);
    }
    WCHAR status_text[256];
    wsprintfW(status_text,
              L"Geminiでレセプトを点検しています(%d件中%d-%d件目 予想残り時間 "
              L"%d分 %d秒)",
              args->target_count, start_idx + 1, start_idx + current_chunk_size,
              remain_sec / 60, remain_sec % 60);
    ...
    for (int i = 0; i < current_chunk_size; i++) {
      ...
      if ((p->Gemini_Response == NULL || strlen(p->Gemini_Response) == 0) &&
          p->Insurance_Flag != 0) {
        ...
        if (error_node) {
          ...
          if (cJSON_IsNumber(code_node)) {
            if (code_node->valueint == 503) {
              MessageBoxW(
                  args->hProgressDlg,
                  L"該当モデルが高負荷のため、処理を実行できません。モデルを変"
                  L"更するか、しばらく待ってからやり直してください。",
                  L"APIエラー", MB_OK | MB_ICONERROR);
              cJSON_Delete(root);
              free(response);
              free(unchecked_indices);
              goto THREAD_ABORT;
              ...
              MessageBoxW(
                  args->hProgressDlg,
                  L"レートリミットに到達しました。解除されるまで待つか、クレジ"
                  L"ットカードを登録して残高をチャージしてください。",
                  L"APIエラー", MB_OK | MB_ICONERROR);
              cJSON_Delete(root);
              free(response);
              free(unchecked_indices);
                ...
                for (int i = 0; i < res_count; i++) {
                  ...
                  if (cJSON_IsNumber(t_id) && cJSON_IsString(def)) {
                    ...
                    if (id_val >= 0 && id_val < unchecked_count) {
                      ...
                      if (p->Gemini_Response)
                        free(p->Gemini_Response);
